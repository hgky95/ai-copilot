...
from resources.QueryConstants import (
    JOIN_PROJECT_AND_TASK,
    ALL_PROJECTS,
    ALL_TASKS,
    ALL_CURRENCIES,
    ALL_MEMBERS,
    ...
    def post(self):
        ...
        if PromptType.FORECAST.value == prompt_type.upper():
            embedchain_app = App.from_config(
                config_path="./config/forecast_config.yaml"
            )
            project_id = "P10"
            embedchain_app.add(
                TASK_JOIN_TASKCOSTITEM_JOIN_COSTITEM,
                # + f"where t.projectId = {project_id}",
                data_type="mysql",
                loader=self.mysql_loader,
            )
            # embedchain_app.add(source='./dataset/cost_item.csv', data_type='csv')
            embedchain_app.add(
                source="https://www.oanda.com/currency-converter/live-exchange-rates/"
            )
        elif PromptType.REPORT.value == prompt_type.upper():
            embedchain_app = App.from_config(config_path="./config/report_config.yaml")
            embedchain_app.add(
                JOIN_PROJECT_AND_TASK, data_type="mysql", loader=self.mysql_loader
            )
